...
class Account:
    def __init__(self, privKey: string):
        ...
        pk = keys.PrivateKey(privKey.encode('utf-8'))
        print(pk.public_key)
        self.privKey = privKey

    def generate_message(self, steps):
        msg = Message(steps)
...
class DataFrame:
    def __init__(self, message: Message, privKey : bytes):
        ...
        self.deviceId = self.publicKey[:20]
        self.cryptography = "ECC"
...
def publish(client, account : Account, topic : string):
    ...
    steps = steps + 10
    data = account.generate_message(steps)
